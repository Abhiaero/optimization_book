...
# Define the Perm Function
def perm_function(x1, x2, beta=0.5):
    D = 2  # Number of dimensions for simplicity
    indices = np.arange(1, D + 1)
    term1 = sum((j + beta) * (x1**j - 1) for j in indices)
    term2 = sum((j + beta) * (x2**j - 1) for j in indices)
    return term1**2 + term2**2

# Generate grid
x1_vals = np.linspace(-2, 2, 200)  # D = 2 implies [-2, 2]
...
optimal_z = perm_function(optimal_x1, optimal_x2)
