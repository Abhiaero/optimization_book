...
# Define the Bohachevsky Function (Variant 1)
def bohachevsky_function(x, y):
    term1 = x**2 + 2*y**2
    term2 = -0.3 * np.cos(3 * np.pi * x)
    term3 = -0.4 * np.cos(4 * np.pi * y)
    return term1 + term2 + term3 + 0.7

# Generate grid
# Zoomed in to [-1, 1] to visualize the trigonometric modulations
x_vals = np.linspace(-1, 1, 400)
y_vals = np.linspace(-1, 1, 400)
X, Y = np.meshgrid(x_vals, y_vals)
Z = bohachevsky_function(X, Y)

# Global Minimum: f(x*) = 0 at x* = (0, 0)
optimal_x, optimal_y = 0.0, 0.0
optimal_z = bohachevsky_function(optimal_x, optimal_y)
...
ax1.scatter(optimal_x, optimal_y, optimal_z, color='red', s=100, edgecolors='white',
            label=f'Global Min = {optimal_z:.1f}', zorder=5)
ax1.legend(loc='upper right', fontsize=12)

# Adjust view angle to show the quadratic basin and periodic traps
