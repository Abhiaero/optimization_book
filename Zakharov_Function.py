...
# Define the Zakharov Function
def zakharov_function(x1, x2):
    term1 = x1**2 + x2**2
    term2 = (0.5 * (1 * x1 + 2 * x2))**2
    term3 = (0.5 * (1 * x1 + 2 * x2))**4
    return term1 + term2 + term3

# Generate grid
x1_vals = np.linspace(-5, 10, 200)
x2_vals = np.linspace(-5, 10, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = zakharov_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0, 0
optimal_z = zakharov_function(optimal_x1, optimal_x2)
