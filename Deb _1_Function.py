...
# Define the Deb 1 Function
def deb_1_function(x1, x2):
    term1 = np.sin(5 * np.pi * x1)**6
    term2 = np.sin(5 * np.pi * x2)**6
    return -(term1 + term2)

# Generate grid (Domain for Deb 1 is typically [0, 1])
x1_vals = np.linspace(0, 1, 200)
x2_vals = np.linspace(0, 1, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = deb_1_function(X1, X2)

# Optimal solution (picking one of the many equally deep global minima)
optimal_x1, optimal_x2 = 0.1, 0.1
optimal_z = deb_1_function(optimal_x1, optimal_x2)
...
ax2.scatter(optimal_x1, optimal_x2, color='red', s=100, label='Representative Minimum = ($0.1$, $0.1$)')
