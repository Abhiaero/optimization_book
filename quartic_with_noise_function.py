...
# Define the Quartic with Noise Function
def quartic_with_noise_function(x1, x2):
    np.random.seed(0)  # Set seed for reproducibility
    noise = np.random.uniform(0, 1)
    return (1 * x1**4 + 2 * x2**4) + noise

# Generate grid
x1_vals = np.linspace(-1.28, 1.28, 200)
x2_vals = np.linspace(-1.28, 1.28, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = quartic_with_noise_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0, 0
optimal_z = quartic_with_noise_function(optimal_x1, optimal_x2)
...
ax1.zaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.2f}'))

# Set ticks and view angle
ax1.set_xticks(np.arange(-1.28, 1.29, 0.5))
ax1.set_yticks(np.arange(-1.28, 1.29, 0.5))
