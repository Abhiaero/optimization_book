...
def rastrigin_function(x1, x2):
    # n-dimensional: 10n + sum(xi^2 - 10*cos(2*pi*xi))
    return 20 + (x1**2 - 10 * np.cos(2 * np.pi * x1)) + (x2**2 - 10 * np.cos(2 * np.pi * x2))

# Generate grid (Standard domain for Rastrigin is [-5.12, 5.12])
x1_vals = np.linspace(-5.12, 5.12, 250)
x2_vals = np.linspace(-5.12, 5.12, 250)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = rastrigin_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0.0, 0.0
optimal_z = rastrigin_function(optimal_x1, optimal_x2)
...
# Using 15 levels to widen the gaps between contour lines
