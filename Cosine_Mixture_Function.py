...
# Define the Cosine Mixture Function
def cosine_mixture_function(x1, x2, alpha_param=1.0):
    term1 = -alpha_param * (np.cos(2 * np.pi * x1) + np.cos(2 * np.pi * x2))
    term2 = x1**2 + x2**2
    return term1 + term2

# Generate grid (Domain for Cosine Mixture is typically [-1, 1])
x1_vals = np.linspace(-1, 1, 200)
x2_vals = np.linspace(-1, 1, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = cosine_mixture_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0.0, 0.0
optimal_z = cosine_mixture_function(optimal_x1, optimal_x2)
...
# Reduced levels from 30 to 15 to widen the gaps between contour lines
