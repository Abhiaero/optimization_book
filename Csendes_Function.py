...
# Define the Csendes Function
def csendes_function(x1, x2, epsilon=1e-6):
    term1 = (x1**6) * (2 + np.sin(1.0 / (x1 + epsilon)))
    term2 = (x2**6) * (2 + np.sin(1.0 / (x2 + epsilon)))
    return term1 + term2

# Generate grid (Domain for Csendes is typically [-1, 1])
x1_vals = np.linspace(-1, 1, 200)
x2_vals = np.linspace(-1, 1, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = csendes_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0.0, 0.0
optimal_z = csendes_function(optimal_x1, optimal_x2)
