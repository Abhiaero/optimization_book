...
def step_function(x1, x2):
    return (np.abs(x1) + 0.5)**2 + (np.abs(x2) + 0.5)**2

# Generate grid
x1_vals = np.linspace(-5, 5, 500)
x2_vals = np.linspace(-5, 5, 500)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = step_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0, 0
optimal_z = step_function(optimal_x1, optimal_x2)
...
ax1.set_xticks(np.arange(-5, 6, 2))
ax1.set_yticks(np.arange(-5, 6, 2))
