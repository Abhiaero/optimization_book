...
# Define the El-Attar-Vidyasagar-Dutta Function
def el_attar_vidyasagar_dutta_function(x1, x2):
    return ((x1**2 + x2 - 10)**2 +
            (x1 + x2**2 - 7)**2 +
            (x1**2 + x2**3 - 1)**2)

# Generate grid
x1_vals = np.linspace(-500, 500, 200)
x2_vals = np.linspace(-500, 500, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = el_attar_vidyasagar_dutta_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 3.4093, -2.1714
optimal_z = el_attar_vidyasagar_dutta_function(optimal_x1, optimal_x2)
...
ax2.scatter(optimal_x1, optimal_x2, color='red', s=100, label='Optimal Solution = $(3.4093, -2.1714)$')
