...
# Define the Powell Sum Function
def powell_sum_function(x1, x2):
    return abs(x1)**2 + 2 * abs(x2)**3

# Generate grid
x1_vals = np.linspace(-1, 1, 200)
x2_vals = np.linspace(-1, 1, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = powell_sum_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0, 0
optimal_z = powell_sum_function(optimal_x1, optimal_x2)
