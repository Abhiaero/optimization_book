...
# Define the Stretched V Sine Wave Function
def stretched_v_sine_wave_function(x1, x2, alpha=0.5):
    term1 = np.abs(x1 - 1)**(2 + alpha)
    term2 = np.sin(x2 - x1)**2
    return term1 + term2
...
Z = stretched_v_sine_wave_function(X1, X2)

# Optimal solution (approximate, as this function may have no simple closed-form minimum)
optimal_x1, optimal_x2 = 1, 1
optimal_z = stretched_v_sine_wave_function(optimal_x1, optimal_x2)
...
surf = ax1.plot_surface(
    X1, X2, Z, cmap=cm.plasma, edgecolor='k', linewidth=0.2, antialiased=True, alpha=0.7
)
...
ax1.zaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.1f}'))
