...
# Define the Schwefel 2.22 Function
def schwefel_2_22_function(x1, x2):
    abs_sum = np.abs(x1) + np.abs(x2)
    abs_product = np.abs(x1) * np.abs(x2)
    return abs_sum + abs_product

# Generate grid
x1_vals = np.linspace(-10, 10, 200)
x2_vals = np.linspace(-10, 10, 200)
X1, X2 = np.meshgrid(x1_vals, x2_vals)
Z = schwefel_2_22_function(X1, X2)

# Optimal solution
optimal_x1, optimal_x2 = 0, 0
optimal_z = schwefel_2_22_function(optimal_x1, optimal_x2)
